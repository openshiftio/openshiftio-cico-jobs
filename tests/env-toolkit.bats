...
@test "can select patterns" {
    eval "$($ENV_TOOLKIT load -f env_write.json ^MYTEST1$)"
...
@test "can select patterns (no-export)" {
    eval "$($ENV_TOOLKIT load --no-export -f env_write.json ^MYTEST1$)"
    [ "$MYTEST1" = "this_is_a_test1" ]
    [ -z "$MYTEST2" ]
    [ -z "$MYTEST3" ]

    eval "$($ENV_TOOLKIT load --no-export -f env_write.json ^MYTEST)"
    [ "$MYTEST1" = "this_is_a_test1" ]
    [ "$MYTEST2" = "this_is_a_test2" ]
    [ "$MYTEST3" = "this_is_a_test3" ]
}
